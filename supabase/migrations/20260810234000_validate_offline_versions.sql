create or replace function private.resolve_offline_assignment_reference(
  p_owner_id uuid,
  p_reference text
)
returns uuid
...
begin
  ...
  if p_reference like 'local:%' then
    ...
    exception when invalid_text_representation then
      raise exception 'Offline assignment version is invalid';
    end;
    select coalesce(
      operation.result #>> '{assignment,assignment_id}',
      operation.result ->> 'assignment_id'
    )::uuid into resolved_assignment_id
    ...
    if resolved_assignment_id is null then
      raise exception 'Offline assignment version is not synchronized';
    end if;
...
revoke all on function private.resolve_offline_assignment_reference(uuid, text)
from public, anon, authenticated;

create or replace function private.assert_offline_assignment_context(
  p_owner_id uuid,
  p_payload jsonb
)
returns void
...
declare
  ...
  intended_assignment_id uuid;
begin
  if not (p_payload ? 'expected_assignment_id')
    or jsonb_typeof(p_payload -> 'expected_assignment_id') not in ('null', 'string')
  then
    raise exception 'Offline assignment context is invalid';
  end if;
  intended_assignment_id := private.resolve_offline_assignment_reference(
    p_owner_id,
    p_payload ->> 'expected_assignment_id'
  );
  select assignment_id into current_assignment_id
  ...
  where user_id = p_owner_id
    ...
    and active
  for update;
  if current_assignment_id is distinct from intended_assignment_id then
    raise exception 'Offline assignment context changed';
  end if;
end;
$$;

revoke all on function private.assert_offline_assignment_context(uuid, jsonb)
from public, anon, authenticated;
...
begin
  ...
  order by workout.started_at desc, workout.workout_id
  limit 1;
